...
class Brain:
    ...
    def calculate_int_rewards(self, next_obs, batch=True):
        ...
        with torch.no_grad():
            target_feature = self.target_model(norm_obs)
            predictor_feature = self.predictor_model(norm_obs)

        int_reward = torch.mean((predictor_feature - target_feature) ** 2, dim=1).cpu().numpy()
        return int_reward
    ...
    def calculate_rnd_loss(self, obs):
        ...
        # final_loss = None
        # return final_loss
        target_features = self.target_model(obs)
        predictor_features = self.predictor_model(obs)

        errors = (predictor_features - target_features) ** 2
        errors = torch.mean(errors, dim=1)

        mask = torch.rand_like(errors) < self.config["predictor_proportion"]
        masked_errors = errors[mask]

        if masked_errors.numel() == 0:
            final_loss = torch.tensor(0.0, device=self.device)
        else:
            final_loss = masked_errors.mean()
        return final_loss
