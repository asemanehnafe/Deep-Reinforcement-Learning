...
class TargetModel(nn.Module, ABC):
    def __init__(self, state_shape):
        ...
        # self.encodeds_features = nn.Linear(...)
        c, w, h = state_shape
...
class PredictorModel(nn.Module, ABC):
    def __init__(self, state_shape):
        ...
        c, w, h = state_shape
        # Match target model's conv layers
        self.conv1 = nn.Conv2d(c, 32, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1)

        flatten_size = 64 * w * h  # Spatial dimensions are preserved
    ...
    def _init_weights(self):
        ...
        for layer in self.modules():
            if isinstance(layer, (nn.Conv2d, nn.Linear)):
                if isinstance(layer, nn.Linear) and layer == self.fc2:
